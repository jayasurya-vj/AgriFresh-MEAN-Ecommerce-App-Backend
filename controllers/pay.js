    ...
    Cart.find({ creator: req.userData.userId }).populate('item')
      ...
      .then(cartItems => {
        ...
        const session = stripe.checkout.sessions.create({
          ...
          mode: "payment",
          success_url: `https://agrifresh-ecommerce-app.herokuapp.com/api/payment/success?session_id={CHECKOUT_SESSION_ID}`,  //&order=${JSON.stringify(req.body.order)}
          cancel_url: `https://agrifresh-ecommerce-app.herokuapp.com/cart`,
        }).then(data => {
            ...
            .then(cartItems => {
              ...
              order.save().then(  data =>{
                Cart.deleteMany({creator:user._id}).then(deleteResult=>{
                  ...
                  res.redirect("https://agrifresh-ecommerce-app.herokuapp.com/orders?orderplaced=true");
                });                
              });
            });
        }).catch(err => {
